...
# # GOAL: to fit spectra in a window around the line.
#
...
# I use DIFFEV to do astro and dm fits
...
#
normline_upper = normlDMhdata2*1e3
...
# just for checks of Diffev to reduce number of parameters
#change_keyfits(params_name, params_target, keyfits, "TBabs", "nH", False)
#change_keyfits(params_name, params_target, keyfits, "apec", "kT", False)
...
params_fit, bounds_fit, params_name_fit, params_target_fit, keyfits_fit = paramfit(params_jitter, lowers, uppers, params_name, params_target, keyfits)
...
def doFit(fullmodel_xp, params_fit, bounds_fit, params_name_fit, params_target_fit,TypeFit, params, lowers, uppers, keyfits):
    ...
    if TypeFit == "Diffev":
        ...
        #out = optimize.differential_evolution(mymodel_fit.evaluate_folded, bounds_fit, polish = False,
        #                              tol = 1e-3, atol = 0, popsize = 15, init = 'sobol',
        #                             maxiter=10000)
        ...
        PrintModelXspec()
        print("++++++++++")
        #test = np.array([0.18502155, 1.70175559, 4.18291719, 0.31965486, 12.90698327])
        #chi2 = mymodel_fit.evaluate_folded(test)
        #print("Parameters:", test)
        #print("Chi2:", chi2)
    
        
        out = optimize.differential_evolution(mymodel_fit.evaluate_folded, bounds_fit,popsize=12,tol=1e-3,
        ...
        #
        #Careful: params_out contains only fitted parameters, not frozen ones.
        # First create array with correct size.
        params_internalfit = params.copy()
        # Now fill it correctly
        for val, name, attr in zip(params_out, params_name_fit, params_target_fit):
            change_param_value(params_internalfit, params_name, params_target, name, attr, val)
        # since you have changed norm and key(True/False), update XSpec
        mymodel_mw.set_values_all(params_internalfit, lowers, uppers, keyfits)
        mymodel_mw.set_frozen(keyfits)
        #print("++++++++++")
        #PrintModelXspec()
        #print("++++++++++")
        ...
        #m = Minuit(mymodel_fit.evaluate_folded, params_out)
        #m.limits = bounds_fit
    ...
    return (params_out,TS_out,pvalue_out,dof_out,nfit_out)

#19:49 20:14
#

params_astro_fitted,TS_astro,pvalue_astro,dof_astro,nfit_astro = doFit(m_mw, params_fit, bounds_fit, params_name_fit, params_target_fit,TypeFit, params_jitter, lowers, uppers, keyfits)


#Careful: params_astro_fitted contains only fitted parameters, not frozen ones.
# First create array with correct size.
params_astro = params.copy()
# Now fill it correctly
for val, name, attr in zip(params_astro_fitted, params_name_fit, params_target_fit):
    change_param_value(params_astro, params_name, params_target, name, attr, val)
...
print("params_target_fit ",params_astro_fitted)
...
params_DM_fitted,TS_DM,pvalue_DM,dof_DM,nfit_dm = doFit(m_mw, params_fit, bounds_fit, params_name_fit, params_target_fit,TypeFit,params_jitter, lowers, uppers, keyfits)


#Careful: params_DM_fitted contains only fitted parameters, not frozen ones.
# First create array with correct size.
params_DM = params.copy()
# Now fill it correctly
for val, name, attr in zip(params_DM_fitted, params_name_fit, params_target_fit):
    change_param_value(params_DM, params_name, params_target, name, attr, val)
...
print("****************")
printparam(params_DM, lowers, uppers , params_name, params_target, keyfits)
print("****************")
print("****************")
PrintModelXspec()
print("****************")
...
for i, A in enumerate(profile_range):
    ...
    if A < 0.5*DMbf:
        seed =  jitter_params(params_astro, lowers, uppers, keyfits, rel=0.02)
        TS1, p1 = fit_once_from(seed, A)
    else:
        seed = jitter_params(params_DM, lowers, uppers, keyfits, rel=0.02)
        TS1, p1 = fit_once_from(seed, A)
    TS_inside = TS1
    pbest = p1
    ...
    if i >= 2:
        ...
        if TS_inside > TS_pred + up_abs:
            ...
            p_jitter = jitter_params(pbest, lowers, uppers, keyfits, rel=0.05)
