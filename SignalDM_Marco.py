...
# ensure local profiles module is found
...
# Transform bound of sterile nu to bound on ALP
...
def FSinsq2theta(Eline,Fluxlim,Dfactor):
    ...
    vFluxDM = FluxLine(vMdm_gev, Dfactor, xtau, nphoton=1)
    # bound on lifetime
    vbound_tau = xtau*vFluxDM/Fluxlim # in sec
    # see eq.1 of https://arxiv.org/pdf/1911.09120
    # and eq. 4.13 of https://arxiv.org/pdf/1602.04816. See NFW.nb in this folder
...
def Fgagg(Eline,Fluxlim,Dfactor):
    ...
    eVtosec = 1.51927e15 #s-1
    # 1 photon for sterile nu
    vFluxDM = FluxLine(vMdm_gev, Dfactor, xtau, nphoton=2)
    # bound on lifetime
    vbound_tau = xtau*vFluxDM/Fluxlim # in sec
    vGamma_GeV = 1.0/vbound_tau*1/eVtosec*1e-9 # GeV
    vgagg = np.sqrt(64*np.pi*vGamma_GeV/(vMdm_gev**3))
    #vgagg = np.sqrt(64*np.pi/(vbound_tau*(vMdm_gev**3)*1e9*eVtosec))
    #vSinsq2theta_M = 7.2e21*pow(vbound_tau,-1.0)*pow(vMdm_kev,-5.0)
    return (vMdm_kev, vgagg)
...
File_fluxlimit1 = np.loadtxt("/home/jortecal/GitHub/eRosita/1-2_XJorge_Gener/Results/CombinedProfiles/CombinedBounds.dat",skiprows=1)
vEline1 = File_fluxlimit1[:,0]
vFluxlim1 = File_fluxlimit1[:,1]

File_fluxlimit2 = np.loadtxt("/home/jortecal/GitHub/eRosita/2to9FixedLines/Results/CombinedProfiles/CombinedBounds.dat",skiprows=1)
...
ax1.plot(vMdm_kev, vSinsq2theta, color='mediumseagreen')
...
#vMdm_kev_alp, vgagg
ax1.plot(vMdm_kev_alp, vgagg, color='mediumseagreen')
...
ax1.plot(vEline, vFluxlim, color='mediumseagreen')
