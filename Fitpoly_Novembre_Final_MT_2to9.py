...
def doFit(mymodel_mw, mymodel_pb,
    ...
    if TypeFit == "XSpec":
        ...
        out = optimize.differential_evolution(combined.evaluate, combined.bounds, popsize=80,tol=1e-3,
            maxiter=3000,init='sobol',polish=False)
        # out = optimize.differential_evolution(
        #     combined.evaluate,
        #     combined.bounds,
        #     popsize=70,
        #     maxiter=3000,  # or maybe reduce to 1500 if runtime becomes huge
        #     mutation=(0.6, 1.2),
        #     # recombination=0.55,
        #     init='sobol',
        #     strategy='rand1bin',  # or you can try 2–3 different strategies
        #     polish=False,
        #     tol=5e-4,  # maybe tighten later if polishing does fine job
        #     seed=1   # but run multiple independent times
        # )
        params_out = out.x
        TS_out = out.fun
...
# Set the xspec model to the DM best fit
...
# Basically only DM normalization for this model.
# params is not at best fit but this is not relevant
# here because the only fitted parameter in mw/params is DM normalization which is changed in ApproximateBound
params_fit, bounds_fit, params_name_fit, params_target_fit, keyfits_fit = paramfit(params_DM_mw, lowers, uppers, params_name, params_target, keyfits)
#
...
# This is because m_pb is already set at best-fit values and ApproximateBound only change DM norm inside m_mw and evaluates TS
# Check this is putting a print of A,TS values in chi2approx and see that for A=DMbf the TS is the one of previous fit.
# Also as a test change parameters of mw_pb via mymodel_pb.set_values_all(params_pb, lowers_pb, uppers_pb),
# since these array have not been update they do not contain the best-fit parameters
#(no need to update since xspec parameters by defalut are those of last fit unless I change them)
# Now the print gives me wrong values for the TS, which tells me that changing m_pb has an impact on
# the TS evaluated inside chi2approx
#
#
...
for i, A in enumerate(profile_range):
    ...
    if i >= 2:
        ...
        if TS_inside > TS_pred + up_abs:
            ...
            p_jitter_pb = jitter_params(pbest_pb, lowers_pb, uppers_pb, keyfits_pb, rel=0.05)
