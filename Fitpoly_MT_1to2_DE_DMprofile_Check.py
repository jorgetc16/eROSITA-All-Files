...
# Load an h5 file with results of fit astro
Eline = 1.0
outdir = "Check_Reading_h5_FitAstro"
input_h5 = "/home/jortecal/GitHub/eRosita/Fit_1-2/Fit_1-2/output/TM4"
telescope = "TM4"
...
def read_fit_astro(folder):
    ...
    with h5py.File(astro_file, "r") as f:
        ...
        print(f"[h5 DEBUG] TS_astro = {TS_astro}")
        print(f"[h5 DEBUG] nfit_astro = {nfit_astro}")
        print(f"[h5 DEBUG] dof_astro = {dof_astro}")
        print(f"[h5 DEBUG] params_astro_mw shape = {params_astro_mw.shape}")
        print(f"[h5 DEBUG] params_astro_pb shape = {params_astro_pb.shape}")
...
def doFit(mymodel_mw, mymodel_pb,
    ...
    if TypeFit == "XSpec":
        ...
        #Standard
        out = optimize.differential_evolution(combined.evaluate, combined.bounds, popsize=30,tol=1e-3,
            maxiter=5000,init='sobol',polish=False)
        #
        # JORGE -> More power
...
params_astro_mw, params_astro_pb,TS_astro,pvalue_astro,dof_astro,nfit_astro = read_fit_astro(input_h5)
...
print("\n=== DIAGNOSTIC CHECK ===")
print(f"Fit.statMethod: {Fit.statMethod}")
print(f"Expected: cstat")
print(f"\nNumber of spectra loaded: {AllData.nSpectra}")
print(f"Noticed channels (Source 1): {AllData(1).noticed}")

print(f"\nFrozen state MW:")
for name, tgt in zip(params_name, params_target):
    param = getattr(getattr(m_mw, name), tgt)
    print(f"  {name}.{tgt}: frozen={param.frozen}, value={param.values[0]}")

print(f"\nFrozen state PB:")
for name, tgt in zip(params_name_pb, params_target_pb):
    param = getattr(getattr(AllModels(1,"mpb"), name), tgt)
    print(f"  {name}.{tgt}: frozen={param.frozen}, value={param.values[0]}")

print(f"\nCalculated TS: {Fit.statistic}")
print(f"TS from h5: {TS_astro}")
print(f"Difference: {abs(Fit.statistic - TS_astro)}")
