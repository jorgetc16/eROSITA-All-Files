...
# else:
model_dic["_structure"] = f"TBabs * (apec + gaussian)"
...
# %%
# Create the XSpec model (build expression from submodel names so names match params)
# collect submodel names (these are the names used in params_name / Extract_array_dic)
components = []
for top_key, sub_models in model_dic.items():
    if not isinstance(sub_models, dict):
        continue
...
def jitter_params(prev_params, rel=0.02):
...
for val, name, attr in zip(params_DM, params_name_astro, params_target_astro):
    change_param_value(params, params_name, params_target, name, attr, val)
...
for i, A in enumerate(profile_range):
    ...
    if i >= 2:
        ...
        up_abs = 1.0     # upward spike tolerance (C-stat units)
        down_abs = 0.5   # downward dip tolerance
        ...
        if TS_inside > TS_pred + up_abs:
            ...
            reseed_ib_norms_small()
            TS3, p3_pb, p3_mw = fit_once_from(seed_astro_pb, seed_astro_mw, A)
            # Unfreeze back
            freeze_ib_line_energies(False)
            if TS3 < TS_inside:
                TS_inside = TS3; pbest_pb = p3_pb; pbest_mw = p3_mw

        # Downward dip: TS much lower than trend → confirm by jittered retry
        elif TS_inside < TS_pred - down_abs:
            p_jitter_pb = jitter_params(pbest_pb, rel=0.03)
            p_jitter_mw = jitter_params(pbest_mw, rel=0.03)
            TS4, p4_pb, p4_mw = fit_once_from(p_jitter_pb, p_jitter_mw, A)
            if TS4 < TS_inside:
                TS_inside = TS4; pbest_pb = p4_pb; pbest_mw = p4_mw
...
while max_TS_tested < TS_threshold and max_A_tested < normline_upper:
    ...
    for i, A in enumerate(profile_range_new):
        ...
        # Spike/dip detection via local predictor (needs i >= 2)
        if i >= 2:
            logA_prev2 = np.log10(profile_range_new[i-2])
            logA_prev1 = np.log10(profile_range_new[i-1])
            logA_i     = np.log10(A)
            TS_pred = local_predict_ts(logA_prev2, TS_vals[i-2], logA_prev1, TS_vals[i-1], logA_i)
...
print("Initial A_95 ", A_95, " DeltaTS ", TS_inside-min_TS)
#
# Refine a bit more
#
...
best_vals_per_A_pb = [None] * (len(profile_range_fine)+1)
...
for i, A in enumerate(profile_range_fine):
    ...
    if i >= 2:
        ...
        # Upward spike: TS jumped much more than expected trend
        if TS_inside > TS_pred + up_abs:
            print("The point ",i, A, " has to be refined because TS_inside ",TS_inside, " TS_pred ",TS_pred)
            # Retry: freeze line energies and reseed line norms small
            freeze_ib_line_energies(True)
            reseed_ib_norms_small()
            TS3, p3_pb, p3_mw = fit_once_from(seed_astro_pb, seed_astro_mw, A)
            ...
            if TS3 < TS_inside:
                ...
                print("Refinement did not help")
                TS_inside = TS_pred + up_abs
            print("After refinement TS_inside ",TS_inside)
...
TS_inside = Fit.statistic;
print("Final A_95 ", A_95, " DeltaTS ", TS_inside-min_TS);
