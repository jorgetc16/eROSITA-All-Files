...
def extract_profile_bounds(folder):
    ...
    namefile = "UFlux_profiling.dat"
    data = np.column_stack([vEline, vbounds])
...
if __name__ == "__main__":
    if len(sys.argv) == 2 and sys.argv[1].endswith(".h5"):
        ...
        data_path = "/Users/marcotaoso/Documents/2024/eROSITA/xSpec/LMC_test_3deg/Fitpoly/FromJ/XSpec/Model5"
        output_file = os.path.join(data_path, "fit_summary.csv")
        summarize_fit_results(data_path, output_csv=output_file)
        #extract_profile_bounds(data_path)
