...
def summarize_fit_results(folder):
    result_rows = []
    files = sorted(glob.glob(os.path.join(folder, "fit_results_E*.h5")))
    if not files:
    ...
    for file in files:
        try:
            ...
            with h5py.File(file, "r") as f:
                ...
                astro_grp = grp["astro"]
                # Read raw parameter array (no names)
                astro_param_values = astro_grp["params_astro"][:]
                astro_param_values = astro_param_values.astype(float)
                print("len(astro_param_values)",len(astro_param_values))
                TBabsNH2 = np.append(TBabsNH2,astro_param_values[0])
                TAPEC = np.append(TAPEC,astro_param_values[2])
                normAPEC = np.append(normAPEC,astro_param_values[5])
                TPLgamma = np.append(TPLgamma,astro_param_values[11])
...
# Loop over folders
for tm in TM_list:
    folder = f"/Users/marcotaoso/Documents/2024/eROSITA/xSpec/LMC_5deg/Fit_global/Fit_1-2/output/TM{tm}"
