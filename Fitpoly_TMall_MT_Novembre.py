...
#print("****************")
#print("****************")
#printparam(params_jitter_pb, lowers_pb, uppers_pb , params_name_pb, params_target_pb, keyfits_pb)
#print("****************")
...
print("pvalue astro ",pvalue_astro)
...
def fit_once_from(seed_params, A):
    ...
    mymodel_pb.set_values(seed_params)
    ...
    return Fit.statistic, mymodel_pb.get_values()
...
# Parameters of mw that I am considered in the fit (keyfits is True). Basically only DM normalization for this model.
...
for i, A in enumerate(profile_range):
    ...
    if i >= 2:
        ...
        if TS_inside > TS_pred + up_abs:
            ...
            ###reseed_ib_norms_small() #MT: not sure if useful and correct!
        ...
        # Not an issue, I am shifting of a small amount sigma and the other parameters are refitted
            ...
            #p_jitter = jitter_params(pbest, rel=0.03)
            p_jitter = jitter_params(pbest, lowers_pb, uppers_pb, keyfits_pb, rel=0.03)
...
for i, A in enumerate(profile_range_fine):
    ...
    if i >= 2:
        ...
        if TS_inside > TS_pred + up_abs:
            ...
            if TS3 < TS_inside:
                TS_inside = TS3; pbest = p3
            p_jitter = jitter_params(pbest, lowers_pb, uppers_pb, keyfits_pb, rel=0.05)
            TS4, p4 = fit_once_from(p_jitter, A)
            if TS4 < TS_inside:
                TS_inside = TS4; pbest = p4
        elif TS_inside < TS_pred - down_abs:
