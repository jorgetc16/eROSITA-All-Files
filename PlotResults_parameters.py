    ...
    for file in files:
        try:
            ...
            with h5py.File(file, "r") as f:
                ...
                phoindex = np.append(phoindex,dm_param_values[3])
                normpwl = np.append(normpwl,dm_param_values[4])
                norm_DM = np.append(norm_DM,dm_param_values[0])
...
#pb: PL(PhoIndex,norm) + Gaussian_IB(LineE,Sigma,norm)
    ...
    return Eline_array, phoindex, norm_DM, normpwl


#Eline, phoindex,norm_DM, TPLgamma = summarize_fit_results("/Users/marcotaoso/Documents/2024/eROSITA/xSpec/LMC_5deg/Fit_windows/Tests/1-2keV/output/TM1")
#print("Eline ",Eline)
#print("phoindex ",phoindex)
...
# Loop over folders
for tm in TM_list:
    folder = f"/home/jortecal/GitHub/eRosita/LMC5Deg/ResultsNewData/2to9/TM{tm}"
    print(f"Processing {folder}")

    Eline, phoindex, norm_DM, normpwl = summarize_fit_results(folder)
    
    all_Eline.append(Eline)
    all_phoindex.append(phoindex)
    all_norm_DM.append(norm_DM)
    all_normpwl.append(normpwl)


print(all_Eline," shape ",all_norm_DM)
...
#plt.plot(Eline,phoindex, marker='o', linestyle='-', markersize=2)
for tm, Eline, phoindex in zip(TM_list, all_Eline, all_phoindex):
    plt.plot(Eline, phoindex, marker='o', linestyle='-', markersize=2, label=f"TM{tm}")


plt.xlabel(r'E (keV)',fontsize=30)
plt.ylabel(r'phoindex',fontsize=30)
# plt.ylim(1e-3, 1)
plt.xscale('log')
# plt.yscale('log')
...
fig = plt.figure(figsize=(8,7))
ax1 = plt.subplot()
# plt.axhline(y=1e-13,color='gray',linestyle='dotted')
# plt.axhline(y=3.0,color='gray',linestyle='dotted')

#plt.plot(Eline,norm_DM, marker='o', linestyle='-', markersize=2)
for tm, Eline, norm_DM in zip(TM_list, all_Eline, all_norm_DM):
    plt.plot(Eline, norm_DM, marker='o', linestyle='-', markersize=2, label=f"TM{tm}")


plt.xlabel(r'E (keV)',fontsize=30)
plt.ylabel(r'norm_DM',fontsize=30)
# plt.ylim(1e-3, 1)
plt.xscale('log')
plt.yscale('log')
...
#plt.plot(Eline,norm_DM, marker='o', linestyle='-', markersize=2)
for tm, Eline, normpwl in zip(TM_list, all_Eline, all_normpwl):
    plt.plot(Eline, normpwl, marker='o', linestyle='-', markersize=2, label=f"TM{tm}")


plt.xlabel(r'E (keV)',fontsize=30)
plt.ylabel(r'norm powerlaw',fontsize=30)
# plt.ylim(1e-3, 1)
plt.xscale('log')
plt.yscale('log')
