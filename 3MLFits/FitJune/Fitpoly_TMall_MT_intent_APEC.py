...
An = len(ALine)
IBAn = IBn + An
use_astro_lines = False if sys.argv[5] == "4" else True
...
print(f"Using {len(IBLine)} IB lines only" if not use_astro_lines else f"Using {len(IBLine)} IB and {len(ALine)} Astro lines")
...
if sys.argv[5] == "1":
    ...
    startAb = 1 # from the https://github.com/xyzhang/eRASS-spectrum/blob/main/spectral_analysis_basic.ipynb)
    startT = 0.2 #keV
    startidx = 1.8
    ...
    model_dic = {
        ...
        "apec": {
            "apec": {
                ...
                "Abundanc" : pdic(startAb, lowerAb, upperAb, True),
    ...
    gaussian_components = [f"gaussian" for i in range(len(IBALine)-1)]
    ...
    startnH = 6e-2 #1e22 cm-2 (from the https://github.com/xyzhang/eRASS-spectrum/blob/main/spectral_analysis_basic.ipynb) The foreground Milkyway nH value is cm (https://www.swift.ac.uk/analysis/nhtot/index.php)
    startapec_1 = 0.21 #from the https://github.com/xyzhang/eRASS-spectrum/blob/main/spectral_analysis_basic.ipynb)
    startAb_1 = 1 # from the https://github.com/xyzhang/eRASS-spectrum/blob/main/spectral_analysis_basic.ipynb)
    startT_1 = 0.2 #keV
    startapec_2 = 0.2 #from the https://github.com/xyzhang/eRASS-spectrum/blob/main/spectral_analysis_basic.ipynb)
    startAb_2 = 1 # from the https://github.com/xyzhang/eRASS-spectrum/blob/main/spectral_analysis_basic.ipynb)
    startT_2 = 0.06 #keV
    startidx = 1.8
    startpl = normlDMhdata *np.pow(Eline,1.)
    uppernH = startnH*5.
    upperT_1 = startT_1*2.0
    upperapec_1 = startapec_1*2.0
    upperAb_1 = startAb_1*3.0
    upperT_2 = startT_2*2.0
    upperapec_2 = startapec_2*2.0
    upperAb_2 = startAb_2*3.0
    upperidx = 3.0
    upperpl = startpl*10.0
    lowernH = startnH*0.5
    lowerT_1 = startT_1/2.0
    lowerapec_1 = startapec_1/10.0
    lowerAb_1 = startAb_1/3.0
    lowerT_2 = startT_2/2.0
    lowerapec_2 = startapec_2/10.0
    lowerAb_2 = startAb_2/3.0
    loweridx = 1.0
    lowerpl = - upperpl
    ...
    model_dic = {
        ...
        "apec": {
            "apec": {
                "kT": pdic(startT_1, lowerT_1, upperT_1, True), # (value, lower, upper, key)
                "norm" : pdic(startapec_1, lowerapec_1, upperapec_1, True),
                "Abundanc" : pdic(startAb_1, lowerAb_1, upperAb_1, False),
                "Redshift" : pdic(0, 0, 0, False)
                },
            "apec": {
                "kT": pdic(startT_2, lowerT_2, upperT_2, True), # (value, lower, upper, key)
                "norm" : pdic(startapec_2, lowerapec_2, upperapec_2, True),
                "Abundanc" : pdic(startAb_2, lowerAb_2, upperAb_2, False),
                "Redshift" : pdic(0, 0, 0, False)
                }
    ...
    gaussian_components = ["gaussian"] + [f"gaussian" for i in range(len(IBALine)-1)]
    gaussian_sum = " + ".join(gaussian_components)
    model_dic["_structure"] = f"tbabs*(apec + apec + powerlaw + gaussian)+ {gaussian_sum}"    
