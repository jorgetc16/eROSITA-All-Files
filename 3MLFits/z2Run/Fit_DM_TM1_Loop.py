...
dm_data = np.loadtxt("Data/DM_Lines_response.txt", delimiter=",")
...
def selectEminEmax(Eline):
    sigma = interpolator(Eline)*1e-3 / 2.355
...
def SelectLines(Eline,sigma):
    ...
    if(EmaxL>Eallmax):
        EmaxL = Eallmax
    data = np.loadtxt("Data/IBLines.txt",skiprows=0)
    IBenergy = data[:,0]
    IBenergymin = data[:,1]
    IBenergymax = data[:,2]
    data = np.loadtxt("Data/AstroLines.txt",skiprows=0)
    Aenergy = data[:,0]
    Aenergymin = data[:,1]
    Aenergymax = data[:,2]
...
def DoALLFit(nstepsDM):
    ...
    print(f"[FIT  ] E = {xElineDM:.3f} keV | Profiling: {t_profile_end - t_profile_start:.2f} s")
