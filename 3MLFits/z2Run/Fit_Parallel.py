...
path_data = "/home/terol/eROSITA/z2Run/Results/"
nameAstro = "Astro_bf_"
nameDM = "DM_bf_"
...
def preload_common_data():
    # Interpolator
    dm_data = np.loadtxt("/home/terol/eROSITA/z2Run/Data/DM_Lines_response.txt", delimiter=",")
    interpolator = interpolate.interp1d(dm_data[:, 0], dm_data[:, 1], kind="linear", fill_value="extrapolate")

    # ARF
    fileARF = "/home/terol/eROSITA/z2Run/Data/srctoolout_000_SourceProducts_00001_cheesemask_circle_masked/srctoolout_120_ARF_00001.fits"
    arf = fits.open(fileARF)['SPECRESP']
    arfE = (arf.data.field('ENERG_LO') + arf.data.field('ENERG_HI')) / 2.
    aeff = arf.data.field('SPECRESP')

    # Lines
    IB = np.loadtxt("/home/terol/eROSITA/z2Run/Data/IBLines.txt")
    Astro = np.loadtxt("/home/terol/eROSITA/z2Run/Data/AstroLines.txt")
...
def SelectLines(Eline,sigma):
    ...
    if(EmaxL>Eallmax):
        EmaxL = Eallmax
    data = np.loadtxt("/home/terol/eROSITA/z2Run/Data/IBLines.txt",skiprows=0)
    IBenergy = data[:,0]
    IBenergymin = data[:,1]
    IBenergymax = data[:,2]
    data = np.loadtxt("/home/terol/eROSITA/z2Run/Data/AstroLines.txt",skiprows=0)
    Aenergy = data[:,0]
    Aenergymin = data[:,1]
    Aenergymax = data[:,2]
...
def DMFit(mpoly, params, bounds, params_name, params_target, iEDM, Eline, nbins, TS_astro, DiffEv=True):
    ...
    if DiffEv == True:
        out = optimize.differential_evolution(mymodel_all.evaluate_folded, bounds, tol = 1e-4, atol = 0, disp = False, popsize = 60, maxiter=100000)
        params = out.x
        t_de_end = time.time()
...
def chi2(mpoly, params, bounds, params_name, params_target, A, DiffEv=True):
    ...
    mymodel = MTmodel(model=mpoly,params_name=params_name,params_target=params_target, fit=Fit)
    if DiffEv == True:
        out = optimize.differential_evolution(mymodel.evaluate_folded, bounds, atol = 0, popsize = 50)
        params_inside = out.x
...
def compute_upper_limit(mpoly, TS_min, params, bounds, params_name, params_target,
                        A_min=1e-6, A_max=1.0, tol=1e-6):
    try:
        A_upper = optimize.brentq(delta_chi2, A_min, A_max,
                         args=(mpoly, params, bounds, params_name, params_target, TS_min),
                         xtol=tol)
        return A_upper
    except ValueError:
        print("No root found in the interval. Try increasing A_max.")
        return None   
...
def DoALLFit(nstepsDM, xElineDM, sigma, normlDMhdata, energies, iEDM, Emin, Emax):
    ...
    A_95 = compute_upper_limit(mpoly, TS_all, params_astro, bounds, params_name, params_target,
                           A_min=bound_approx/100, A_max=bound_approx*100)
...
def run_fit_for_energy(xElineDM, common_data):
    ...
    try:
        ...
        path = "/home/terol/eROSITA/z2Run/Data/srctoolout_000_SourceProducts_00001_cheesemask_circle_masked/"
        ...
        DoALLFit(nstepsDM, xElineDM, sigma, normlDMhdata, energies, iEDM, Emin, Emax)
        t_fit_end = time.time()
