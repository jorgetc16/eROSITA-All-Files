...
def selectEminEmax(Eline):
    sigma = interpolator(ElineDM)*1e-3 / 2.355
...
def DoALLFit(nstepsDM):
    ...
    t_de_start = time.time()
    TS_astro, params_astro = AstroFit(mpoly, params, bounds, params_name, params_target, iEDM, xElineDM, nbins, DiffEv=True)
    t_de_end = time.time()
    print("Time for Astro fit ",t_de_end-t_de_start," seconds")
    ...
    t_de_start = time.time()
    TS_all, params_all = DMFit(mpoly, params_all, bounds_all, params_name_all, params_target_all, iEDM, xElineDM, nbins, TS_astro, DiffEv=True)
    t_de_end = time.time()
    print("Time for DM fit ",t_de_end-t_de_start," seconds")
    ...
    t_de_start = time.time()
    Profiling(mpoly, bound_approx, nstepsDM, TS_all, params_astro, bounds, params_name, params_target, DiffEv=True)
    t_de_end = time.time()
    print("Time for Profiling ",t_de_end-t_de_start," seconds")
...
############### RELEVANT ARRAYS ##############################
nDM= 1
EDMmin = 8.5 #energies[0]
EDMmax = 8.5 #energies[-1]
