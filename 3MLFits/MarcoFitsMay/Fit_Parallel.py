...
def preload_common_data():
    # Interpolator
    dm_data = np.loadtxt("/home/jortecal/GitHub/eRosita/3MLFits/DM_Lines_response.txt", delimiter=",")
    interpolator = interpolate.interp1d(dm_data[:, 0], dm_data[:, 1], kind="linear", fill_value="extrapolate")

    # ARF
    fileARF = "/home/jortecal/GitHub/eRosita/LMC3Deg_JorgePC/srctoolout_000_SourceProducts_00001_cheesemask_circle_masked/srctoolout_120_ARF_00001.fits"
    arf = fits.open(fileARF)['SPECRESP']
    arfE = (arf.data.field('ENERG_LO') + arf.data.field('ENERG_HI')) / 2.
    aeff = arf.data.field('SPECRESP')

    # Lines
    IB = np.loadtxt("/home/jortecal/GitHub/eRosita/3MLFits/MarcoFitsMay/IBLines.txt")
    Astro = np.loadtxt("/home/jortecal/GitHub/eRosita/3MLFits/MarcoFitsMay/AstroLines.txt")
